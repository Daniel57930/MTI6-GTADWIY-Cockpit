...
import { getEnv, isBrowser } from "../../src/lib/envGuard";
...
export async function getProvider() {
  ...
  if (!rpc) throw new Error("No provider available: window.ethereum not found and REACT_APP_RPC_URL not set");
...
export async function buildAndSendSwap({ tokenIn, tokenOut, fee = 3000, recipient, amountIn, amountOutMinimum = 0, deadline = Math.floor(Date.now() / 1000) + 60 * 20, sqrtPriceLimitX96 = 0 }) {
  if (!isBrowser()) throw new Error("Swap requires browser environment");
  if (!window?.ethereum) throw new Error("No injected wallet found (window.ethereum). Please install MetaMask or another Web3 wallet.");
